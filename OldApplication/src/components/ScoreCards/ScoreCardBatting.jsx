        ...
        <div ref={divRef} className={`p-4 relative rounded-lg shadow-lg w-96 flex flex-row-reverse`} style={{ backgroundColor: mainBgColor, color: textColor }}>
          ...
          <div className="w-full flex flex-col mt-8">
            <div className="flex justify-between items-center w-full">
              <h2 className="text-lg w-full font-bold text-center">{team1 || 'Team 1'}</h2>
              <span className="font-bold">vs</span>
              <h2 className="text-lg w-full font-bold text-center">{team2 || 'Team 2'}</h2>
            </div>

            <ul className="mt-1 py-1 w-full px-5" style={{ backgroundColor: ulBgColor }}> {/* Apply new background color */}
              {playerNames.map((name, index) => (
                <li key={index} className={`text-left font-bold uppercase tracking-wide pl-2  mb-0.5`} style={{ backgroundColor: liBgColor, color: textColor }}>
                  {name || `Player ${index + 1}`}
                </li>
              ))}
            </ul>
