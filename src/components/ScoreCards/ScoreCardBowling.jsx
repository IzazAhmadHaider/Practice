...
const ScoreCardBowling = () => {
  ...
  function calculateEconomy(runsConceded, ballsBowled) {
    const oversBowled = ballsBowled / 6;
    if (oversBowled <= 0) {
      return 0; // Avoid division by zero
    }
    const economyRate = runsConceded / oversBowled;
    return Math.round(economyRate * 10) / 10; // Round to 1 decimal place
  }
  const handlePlayerCountChange = (event) => {
    const count = parseInt(event.target.value, 10);
    if (count > 0) {
      setNumPlayers(count);
      setPlayers(Array(count).fill(null).map(() => ({
        name: "",
        overs: 0,
        score: 0,
        Wickets: 0,
    ...
    <div className="p-4 font-SairaCondensed flex w-full space-y-4">
      <div className="flex max-lg:flex-col-reverse w-full space-x-5">
        <div className="flex flex-col w-full max-w-md space-y-4">
          ...
          />
          <div>
            <label htmlFor="numPlayers" className="block mb-2">
              Please select Number Of Bowlers
            </label>
            <input
              id="numPlayers"
              type="range"
            ...
            />
            <div className="flex justify-between w-[85%] text-xs">
              {[...Array(9)].map((_, i) => (
                <span key={i}>{i + 3}</span>
              ))}
            </div>
                ...
                {visiblePlayerIndex === index && (
                  <div ref={el => playerInputRefs.current[index] = el} className="space-y-2 absolute z-50 bg-slate-400 p-1 min-w-[30vw] max-md:w-[90vw] outline">
                    ...
                    <input
                      ...
                      value={player.overs}
                      onChange={(e) => handlePlayerChange(index, "overs", e.target.value)}
                      className="block w-full p-1 border border-gray-300 rounded"
                    />
                    <input
                      type="number"
          ...
          >
            ...
            <div className="flex w-full flex-col mt-8">
              ...
              <div className="mt-1 py-1 w-full px-2 " style={{ backgroundColor: ulBgColor + "10" }}>
                {/* Table header */}
                <div className="grid grid-cols-6 font-semibold text-xs text-center uppercase border-b border-gray-300 pb-2" style={{ color: textColor }}>
                  <span className="col-span-2">Name</span>
                  <span className="">Runs</span>
                  <span className="">overs</span>
                  <span className="">Wickets</span>
                  <span className="">Econ</span>
                </div>
                ...
                <ul className="mt-2 space-y-1">
                  {players.map((player, index) => (
                    <li
                      key={index}
                      className="grid grid-cols-6 text-[10px] text-center font-medium uppercase"
                      style={{ backgroundColor: liBgColor, color: textColor }}
                    >
                      <span className="col-span-2 tracking-wider">{player.name || `Player ${index + 1}`}</span>
                      <span className="">{player.score}</span>
                      <span className="">{player.overs}</span>
                      <span className="">{player.Wickets}</span>
                      <span className="">{calculateEconomy(player.score, player.overs)}</span>
                    </li>
                  ))}
                </ul>
